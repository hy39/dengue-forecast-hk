...
#formula <- RISK ~ (1 | AREA) + T5 + T3 + T7 + T4 + R5 + R6 + R4 # Best model 
formula <- RISK ~ (1 | AREA) + T5 + T3 + T7 + T4 + R5 + R4 + R8 # Alternative model
# notes?
#formula <- NULL
...
exit() # continue to run LOOCV
 ...
 marginalVar <- "R6"
 outputFile <- T
 outputFilePath <- "../../marginal_effect_R6.tiff"
 ...
 p <- plot(p) + labs(x = paste(marginalVar, "", sep=" "), y="Relative Risk", title="")
 if (outputFile) {
